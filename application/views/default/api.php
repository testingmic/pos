						...
						if ($userData->record_type == "new-record") {
							...
							if ($checkData != false && $checkData[0]->userTotal == '1') {
                                ...
                                if(!$continue) {
    								...
    								$query = $posClass->updateData(
    									"users",
    									"name='{$userData->fullName}', gender='{$userData->gender}', email='{$userData->email}', 
    									phone='{$userData->phone}', access_level='{$userData->access_level}', 
    									".(!empty($newPassword) ? "password='{$newPassword}'," : null)."
    									branchId='{$userData->branchId}', daily_target='{$userData->daily_target}', 
    									weekly_target='{$userData->weekly_target}', monthly_target='{$userData->monthly_target}'",
