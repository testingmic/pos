...
function nonWorkingDay($message = "Please note that the Point of Sale is Closed for Today.") {
  ...
  return '<div class="no-work-placeholder main-body-loader" style="display: flex;">
          <div class="no-work-content text-center">
              <p class="alert alert-warning text-white" style="border-radius:0px">'.$message.' Come back on <strong>'.
                $posClass->stringToArray($clientData->shop_opening_days)[0]
              .'</strong></p>
          </div>
      </div>';
}

// confirm if current date is in allowed dates
$validDate = in_array(date("l"), $posClass->stringToArray($clientData->shop_opening_days));
